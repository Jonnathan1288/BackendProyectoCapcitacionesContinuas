...
public class ParticipanteAprobadosController {
    ...
    public ResponseEntity<?> findALlParticipantesAprovadosByIdCursos(@RequestBody List<Integer> courses){
        try {
            List<ListApproved> findList = participantesAprobadosService.findALlParticipantesAprovadosByIdCursos(courses);
            if(findList != null){
    ...
    @PostMapping("/all/participantesAprovados/andupdate/findbyIdCourse")
    public ResponseEntity<?> findALlParticipantesAprovadosAndUpdateByIdCursos(@RequestBody List<Integer> courses){
        try {
            List<ParticipantesAprobados> findList = participantesAprobadosService.findALlParticipantesAprovadosAndUpdateByIdCursos(courses);
            if(findList != null){
                return new ResponseEntity<>(findList, HttpStatus.OK);
            }
            return new ResponseEntity<>("PARTICIPANTD APPROVEDS NOt FOUND",HttpStatus.NOT_FOUND);
...
//-----------------------------------------------------NEW METHODS..
    //this method don´t fuction in this update..
